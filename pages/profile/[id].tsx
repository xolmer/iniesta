    ...
    <div className="w-full">
      <div className="flex gap-6 md:gap-10 mb-4 bg-white w-full">
        <div className="w-16 h-16 md:w-32 md:h-32 ">
          <Image src={user.image} alt={"Avatar"} width={120} height={120} className="rounded-full" layout="responsive" />
        </div>
      ...
      <div>
        ...
        <div className="flex gap-6 flex-wrap md:justify-start">{videosList.length > 0 ? videosList.map((post: Video, idx: number) => <VideoCard key={idx} post={post} />) : <NoResults text={`No ${showUserVideos ? "" : "Liked"} Videos Yet`} />}</div>
      </div>
    </div>
  );
};
