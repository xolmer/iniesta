...
const Search = ({ videos }: { videos: Video[] }) => {
  ...
  const searchedAccounts = allUsers?.filter((user: IUser) => user.userName.toLowerCase().includes(search));
  ...
  return (
    <div className="w-full  ">
      ...
      {isAccounts ? (
        ...
        <div className="md:mt-16 flex flex-wrap gap-6 md:justify-start ">{videos.length ? videos.map((post: Video, idx: number) => <VideoCard post={post} key={idx} />) : <NoResults text={`No Video Results for ${search}`} />}</div>
      )}
    </div>
  );
};
